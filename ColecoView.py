...
class ColecoView(BinaryView):
    ...
    def is_valid_cartridge_header(self, data):
        ...
        if cart not in [0xAA55, 0x55AA]:
            return False
        if (start_game & 0xF000) != 0x8000:
            return False
        return True
    ...
    def get_rom_type(self, data):
        ...
        if len(data.read(0, 0xC000)) == 0xC000:
            if self.is_valid_cartridge_header(data.read(0x8000, 12)):
                if (
                    data.read(0, 16)
                    == "\x31\xb9\x73\xc3\x6e\x00\xff\xff\xc3\x0c\x80\xff\xff\xff\xff\xc3"
                ):
    ...
    def init(self):
        ...
        self.add_auto_segment(
            0x6000, 0x2000, 0, 0, SegmentFlag.SegmentReadable | SegmentFlag.SegmentWritable
        )
        # bios and cartridge ROM are in their natural place for system images
        if self.romType == "system":
            self.add_auto_segment(
                0x0, 0x2000, 0, 0x2000, SegmentFlag.SegmentReadable | SegmentFlag.SegmentExecutable
            )
            self.add_auto_segment(
                0x8000,
                0x4000,
        ...
        #
            ...
            self.add_auto_segment(
                0x0, 0x2000, 0, 0, SegmentFlag.SegmentReadable | SegmentFlag.SegmentExecutable
            )
            self.add_auto_segment(
                0x8000,
                0x4000,
