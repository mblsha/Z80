    ...
    def init(self):
        ...
        for line in self.parent_view.read(0, len(self.parent_view)).split(b"\x0a"):
            ...
            if match:
                ...
                assert flags == 0
                assert addr == 0
                log_info(f"adding _CODE [{addr:X}, {addr + size:X})")
            ...
            if match:
                ...
                log_info(f"writing to {addr:X}: {match.group(2)}")
            ...
            if match:
                ...
                if name not in [".__.ABS.", ".  .ABS"]:
                    addr = int(addr, 16)
                    log_info(f"saving symbol {name} @ {addr:X}")
                    syms.append((name, addr))
                    continue

        assert have_code
        for name, addr in syms:
            log_info(f"applying symbol {name} @ {addr:X}")
